...
class Statement {
  ...
  prepBindExecute(sql, params, callback) {
    if (typeof sql !== 'string' || Array.isArray(params) === false){
      throw new TypeError('[node-odbc]: sql string formatted incorrectly in call to `prepBindExecute`')
    }
    if (!this.odbcStatement) {
      throw new Error(Statement.STATEMENT_CLOSED_ERROR);
    }
    return this.odbcStatement.prepBindExecute(sql, params, callback)
  }
